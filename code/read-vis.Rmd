...
p4 <- ggplot(pie2, aes(x="", y=value, fill=Group)) +
  ...
  coord_polar("y", start=0)+
  theme_void()+
  theme(legend.position="right", legend.key.size = unit(1,'cm'), legend.text = element_text(size = 10)) +
  theme(legend.title= element_blank()) +
  geom_label_repel(aes(y=ypos, label = value),
                   segment.colour = "black",size = 7, nudge_x = 0.5, show.legend = FALSE) +
...
p1p4 <- ggpubr::ggarrange(p1,p4, nrow = 1, widths = c(1,0.5), align = "h")
p1p4
